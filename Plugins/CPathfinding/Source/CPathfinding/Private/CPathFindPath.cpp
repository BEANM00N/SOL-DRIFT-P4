...
void UCPathAsyncFindPath::Activate()
{
    if (!IsValid(Request.VolumeRef))
    {
       ...
       SetReadyToDestroy();
       // Fix 2: Removed "RemoveFromRoot()" here.
    }
    else
    {
       // Fix 1: Changed BindUFunction to BindUObject. No reflection system crashes!
       Request.OnPathFound.BindUObject(this, &UCPathAsyncFindPath::OnPathFound);
...
void UCPathAsyncFindPath::OnPathFound(FCPathResult& PathResult)
{
    ...
    SetReadyToDestroy();
    // Fix 2: Removed "RemoveFromRoot()" here as well.
}